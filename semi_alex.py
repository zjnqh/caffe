...
if __name__ == '__main__':
	...
	sub_niter_semi,sub_niter_super=10,100
	...
	index2=Indexing(TOTAL_NUM,batchsz,2)
	...
	for it in xrange(0,niter):
		...
		for sit in xrange(sub_niter_semi):
			...
			label=labelset[index1.fix()]
			print "label",label[:10]
			print net.blobs['label'].data[:5]
			...
			# 	net.layers[layer_id].use_data.data.fill(0)
			# net.layers[semi_layer].use_data.data.fill(0)
			# solver.step_backward()
			# solver.apply_update()
			solver.step_extra()
