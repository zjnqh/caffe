...
void LMDBTransaction::Get(const string& key, string& value) {
  ...
  // value.data()=reinterpret_cast<char*>(mdb_value.mv_data );
  // value.size()=mdb_value.mv_size;
  // value=string(reinterpret_cast<char*>(mdb_value.mv_data ),mdb_value.mv_size);
  // memcpy(value.data(),);
  // string nstr(mdb_value.mv_data,mdb_value.mv_size);
  value.assign(reinterpret_cast<const char*>(mdb_value.mv_data),mdb_value.mv_size);
